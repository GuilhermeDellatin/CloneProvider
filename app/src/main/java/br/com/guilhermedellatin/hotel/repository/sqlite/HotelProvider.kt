...
class HotelProvider : ContentProvider() {
    ...
    override fun getType(uri: Uri): String? {
       val uriType = sUriMatcher.match(uri)
        return when (uriType) {
            TYPE_HOTEL_DIR -> return "${ContentResolver.CURSOR_DIR_BASE_TYPE}/br.com.guilhermedellatin.hotel"
            TYPE_HOTEL_ITEM -> return "${ContentResolver.CURSOR_ITEM_BASE_TYPE}/br.com.guilhermedellatin.hotel"
            else -> null
        }
    }
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
        ...
        val id: Long
        when (uriType) {
            TYPE_HOTEL_DIR -> id = sqlDB.insertWithOnConflict(TABLE_HOTEL, null, values, SQLiteDatabase.CONFLICT_REPLACE)
            else -> throw IllegalArgumentException("URI não suportada: $uri")
        }
        context?.contentResolver?.notifyChange(uri, null)
        //context.contentResolver.notifyChange(uri, null)
    ...
    override fun query(
        ...
        val cursor: Cursor
        when (uriType) {
            TYPE_HOTEL_DIR -> cursor = queryBuilder.query(db, projection, selection, selectionArgs, null, null, sortOrder)
            TYPE_HOTEL_ITEM -> {
    ...
    override fun update(
        ...
        val rowsAffected = when (uriType) {
            TYPE_HOTEL_DIR ->
                sqlDB.update(TABLE_HOTEL, values, selection, selectionArgs)
            TYPE_HOTEL_ITEM -> {
                val id = uri.lastPathSegment
                sqlDB.update(TABLE_HOTEL, values, "$COLUMN_ID= ?", arrayOf(id))
            }
            else -> throw IllegalArgumentException("URI não suportada: $uri")
        }
        context?.contentResolver?.notifyChange(uri, null)
    ...
    companion object {
        private const val AUTHORITY = "br.com.guilhermedellatin.hotel"
